...
if __name__ == '__main__':
    ...
    # 原始分数（未归一化）
    logits = torch.tensor([
        [0.7, 0.1, 0.15, 0.05],  # 样本1：最高分 0.7 在索引1（猫），真实：猫  ✅️
        [0.2, 0.6, 0.15, 0.05],  # 样本2：最高分 0.6 在索引0（狗），真实：狗  ✅️
        [0.1, 0.2, 0.5, 0.2],    # 样本3：最高分 0.5 在索引2（鸟），真实：鸟  ✅️
        [0.05, 0.1, 0.05, 0.8],  # 样本4：最高分 0.8 在索引3（鱼），真实：鱼  ✅️
    ], dtype=torch.float32)

    loss = criterion(logits, y_true)
    print(f"PyTorch多分类交叉熵损失值: {loss.item():.4f}")       # 1.0184
    ...
    # 原始分数（未归一化）
    logits = tf.constant([
        [0.1, 0.7, 0.15, 0.05],     # 样本1：最高分 0.7 在索引1（狗），真实：猫  ❌️
        [0.6, 0.2, 0.15, 0.05],     # 样本2：最高分 0.6 在索引0（猫），真实：狗  ❌️
        [0.1, 0.2, 0.5, 0.2],       # 样本3：最高分 0.5 在索引2（鸟），真实：鸟  ✅️
        [0.05, 0.1, 0.05, 0.8],     # 样本4：最高分 0.8 在索引3（鱼），真实：鱼  ✅️
    ], dtype=tf.float32)

    loss = loss_fn(y_true, logits)
    print(f"TensorFlow多分类交叉熵损失值: {loss.numpy():.4f}")   # 1.2684
